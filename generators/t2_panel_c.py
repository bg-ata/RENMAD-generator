"""
Template C — "Squares"  1920 × 1080

Two-phase rendering:
  Phase 1: background, shapes, square photos (→ pptx_canvas)
  Phase 2: session text for PNG only
  PPTX:    pptx_canvas + editable text boxes = zero doubling
"""
import io, math, re
...
def _top_overlay(w, h, alpha_max=220):
    ...
    for y in range(h):
        if y <= solid_to:
            ...
            t = (y - solid_to) / (h - solid_to)
            a = int(alpha_max * (1 - t ** 0.7))
        d.line([(0, y), (w - 1, y)], fill=(8, 8, 14, a))
    return grad


def _square_photo(photo_img, size, theme_rgb, radius=PHOTO_R, border=BORDER_W):
    total = size + border * 2
    out   = Image.new("RGBA", (total, total), (0, 0, 0, 0))
    d     = ImageDraw.Draw(out)
    d.rounded_rectangle([0, 0, total-1, total-1],
                        radius=radius + border, fill=(*theme_rgb, 255))
    raw  = ImageOps.fit(photo_img.convert("RGBA"), (size, size), Image.LANCZOS)
    mask = Image.new("L", (size, size), 0)
    ImageDraw.Draw(mask).rounded_rectangle([0, 0, size-1, size-1],
                                            radius=radius, fill=255)
    raw.putalpha(mask)
    out.paste(raw, (border, border), mask=raw.split()[3])
    return out


def _silhouette_square(size, theme_rgb, radius=PHOTO_R, border=BORDER_W):
    total = size + border * 2
    out   = Image.new("RGBA", (total, total), (0, 0, 0, 0))
    d     = ImageDraw.Draw(out)
    d.rounded_rectangle([0, 0, total-1, total-1],
                        radius=radius + border, fill=(*theme_rgb, 255))
    d.rounded_rectangle([border, border, border+size-1, border+size-1],
                        radius=radius, fill=(32, 32, 40, 255))
    tr, tg, tb = theme_rgb
    col = (tr, tg, tb, 180)
    cx  = total // 2
    hrad = int(size * 0.13); hy = border + int(size * 0.28)
    d.ellipse([cx-hrad, hy-hrad, cx+hrad, hy+hrad], fill=col)
    bw = int(size * 0.26); by = hy + hrad + 2
    d.ellipse([cx-bw, by, cx+bw, by + int(size * 0.46)], fill=col)
    return out
...
def generate(session: dict, theme: dict, language: str = "es",
    ...
    overlay = _top_overlay(W, TITLE_H, alpha_max=222)
    canvas.paste(Image.new("RGB", (W, TITLE_H), (8, 8, 14)),
    ...
    # "WEBINAR GRATUITO" badge — branding, draw now
    wb_f  = _f("bold", 28)
    ...
    if title_text:
        text_els.append({'x': pad_l, 'y': title_start_y,
                         ...
                         'text': title_text, 'size': 72, 'bold': True,
    ...
    if session.get("date_str"):
        ...
        text_els.append({'x': date_x0 + 14, 'y': ty + 14,
                         ...
                         'text': date_text, 'size': 32, 'bold': True,
                         'color': (255, 255, 255)})
        rx += 14
    ...
    if session.get("time_str"):
        ...
        text_els.append({'x': time_x0 + 14, 'y': ty + 14,
                         ...
                         'text': time_text, 'size': 32, 'bold': True,
                         'color': (255, 255, 255)})
        rx += 14
    ...
    photo_size  = min(col_w - 60, MAX_PHOTO)
    total_photo = photo_size + BORDER_W * 2
    ...
    for i, (spk, pill) in enumerate(zip(speakers, logo_pills)):
        ...
        # Square photo. Real photos are NOT baked into the background — they
        # are pasted in Phase 2 for the PNG and become movable, re-croppable
        # picture shapes in the PPTX.
        photo_x = cx - total_photo // 2
        photo_y = SPLIT_Y - total_photo
        sq = None
        if spk.get("photo"):
            try:
                sq = _square_photo(spk["photo"], photo_size, theme_rgb)
            except Exception:
                sq = None
        if sq is not None:
            sd = photo_size + BORDER_W   # outline centred on edge → outer matches PNG
            photo_els.append({'pil': sq, 'px': photo_x, 'py': photo_y,
                              'x': photo_x + BORDER_W / 2,
                              'y': photo_y + BORDER_W / 2,
                              'w': sd, 'h': sd,
                              'radius': PHOTO_R + BORDER_W / 2,
                              'src': spk["photo"]})
        else:
            sil = _silhouette_square(photo_size, theme_rgb)
            canvas.paste(sil, (photo_x, photo_y), mask=sil.split()[3])
    ...
    for pe in photo_els:
        canvas.paste(pe['pil'], (pe['px'], pe['py']), mask=pe['pil'].split()[3])
    for le in logo_els:
